...
gulp.task('validate-forstore', ['validate-xml', 'validate-highResolutionIconUrl'], function(){
  ...
  parser.parseString(xml, function(err, manifestJson){
    ...
    for (var i = 0; i < manifestJson.OfficeApp.FormSettings[0].Form.length; i++) {
      var formElement = manifestJson.OfficeApp.FormSettings[0].Form[i];
      if (formElement.DesktopSettings[0].SourceLocation[0].$.DefaultValue.indexOf('https://') !== 0) {
        console.log(chalk.red('ERROR: 5.7. Apps and add-ins must be secured with a valid ' +
          'and trusted SSL certificate (HTTPS).'));
        console.log(chalk.blue('FIX: Change the URL of the Form for ' + formElement.$['xsi:type']
          + ' at OfficeApp/FormSettings/Form/DesktopSettings/SourceLocation/DefaultValue (' +
          formElement.DesktopSettings[0].SourceLocation[0].$.DefaultValue) + ') to use https://');
      }
    }
